...
本模块幂等: 入口/防缓存脚本在→不动; 缺了→自动补回。被 pre_push_check.py 在推送前调用,
...
CB_MARK = "my_preds/latest.html?t="
CB_JS = (
    "<script>\n"
    "// 克劳德交叉预测入口: 每次加载追加时间戳, 彻底绕过缓存\n"
    "(function(){var ts=Date.now();var a=document.querySelectorAll('a[href^=\"my_preds/latest.html\"]');"
    "for(var i=0;i<a.length;i++){a[i].href='my_preds/latest.html?t='+ts;}})();\n"
    "</script>"
)


def ensure(idx_path=None) -> bool:
    """补回入口 + 防缓存脚本。返回 True=本次做了修复。"""
    ...
    # 3) 防缓存脚本: 保证入口每次点击都绕过缓存
    if CB_MARK not in html and "</body>" in html:
        html = html.replace("</body>", CB_JS + "\n</body>", 1)
