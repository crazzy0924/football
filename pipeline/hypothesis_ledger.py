...
DECIDED = {
    ...
    "BTTS/大球偏高": (
        ...
        "**单场轶事在聚合口径下会完全反向**"),
    # ── 2026-09-16 外部技能 football-analysis 的断言, 用本系统语料证伪 ──
    "极端低赔陷阱(外部断言)": (
        "已否决-方向相反",
        "断言: 热门赔率<=1.15 时实际打出率低于隐含约 40%。"
        "实测 13168 场(Pinnacle 去水)该档实际命中 91.75% vs 隐含 86.71%, "
        "**高于**隐含 +5.0pp(相对 +5.8%), ROI +1.9% —— 既不是陷阱也没低 40%。"
        "全部分档实际命中均 >= 隐含, 符合已知的'热门被低估/冷门被高估'偏斜。"
        "复算: python tools/test_extreme_favorite.py"),
    "总进球被系统性低估": (
        "已裁决-不修",
